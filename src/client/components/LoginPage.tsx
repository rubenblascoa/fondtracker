...
export function LoginPage({ onAuth, onSuccess, onSwitchToRegister }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!cleanId) {
      setError("Introduce tu email o usuario");
      return;
    }
    if (!password) {
    ...
    setLoading(true);
    try {
      const res = await api.login({ identifier: cleanId, password });
      setToken(res.token);
      if (onAuth) onAuth();
        ...
        <div className="hidden lg:flex w-full lg:w-[50%] p-8 sm:p-12 relative overflow-hidden flex-col justify-center bg-[var(--color-ink-2)]/30">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[var(--color-fg-4)] text-sm mb-12 flex items-center gap-2 font-medium">
              ...
              Acceso seguro a tu panel de inversiones
            </p>
        ...
        <div className="w-full lg:w-[50%] p-5 sm:p-12 bg-[var(--color-ink-1)] border-t lg:border-t-0 lg:border-l border-[var(--color-ink-3)] flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="block text-[13px] text-[var(--color-fg-1)] font-medium">Email o Usuario</label>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-[var(--color-ink-2)] border border-[var(--color-ink-3)] focus:border-[var(--color-accent)] text-[var(--color-fg-1)] text-sm pl-11 pr-10 py-3.5 rounded-xl outline-none transition-all placeholder:text-[var(--color-fg-5)]"
                  placeholder="••••••••"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-fg-4)] hover:text-[var(--color-fg-1)] transition-colors p-1"
                  tabIndex={-1}
                >
                  {showPassword ? (
          ...
          <div className="mt-8 pt-6 border-t border-[var(--color-ink-3)] flex justify-between items-center">
             <span className="text-[13px] text-[var(--color-fg-4)]">¿No tienes una cuenta?</span>
             <button
